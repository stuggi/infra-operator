...
// HostNameStr is used for validation of a hostname.
...
type AllocationRange struct {
	...
	// +kubebuilder:validation:Optional
	// ExcludeAddresses a set of IPs that should be excluded from range
	// TODO: add validation to webhook
...
type NetConfigStatus struct {
	...
	Reservations map[string]NetReservation `json:"reservations,omitempty" optional:"true"`
}

// NetReservation - Reservations per subnet
type NetReservation struct {
	Subnet NetNameStr `json:"subnet,omitempty" optional:"true"`
	Count  int        `json:"count,omitempty" optional:"true"`
}
...
// IsReady returns true if NetConfig reconciled successfully
func (instance NetConfig) IsReady() bool {
...
func (s NetConfigStatus) GetConditions() condition.Conditions {
	return s.Conditions
}
