...
var _ = Describe("IPSet controller", func() {
	...
	When("a GetDefaultIPSetSpec IPSet gets created", func() {
		...
		// validate that NetConfig is there
		It("should have created an IPSet", func() {
			...
			Eventually(func(g Gomega) {
				...
				g.Expect(res.Spec.Reservation).To(HaveLen(1))
				g.Expect(res.Spec.Reservation["net-1"]).To(Not(BeNil()))
				net1Res := res.Spec.Reservation["net-1"]
			...
			Eventually(func(g Gomega) {
				...
				for _, ipSetRes := range ipSet.Status.Reservation {
					netRes, ok := res.Spec.Reservation[string(ipSetRes.Network)]
