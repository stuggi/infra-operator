...
var _ = Describe("DNSMasq controller", func() {
	...
	When("A DNSMasq is created", func() {
		...
		It("creates a Deployment for the service", func() {
			...
			Expect(depl.Spec.Selector.MatchLabels).To(Equal(map[string]string{"service": "dnsmasq"}))
			...
			Expect(container.LivenessProbe.TCPSocket.Port.IntVal).To(Equal(int32(53)))
			Expect(container.ReadinessProbe.TCPSocket.Port.IntVal).To(Equal(int32(53)))
		...
		/*
			When("the Deployment has at least one Replica ready", func() {
				...
				It("reports that the Deployment is ready", func() {
					th.ExpectCondition(
						dnsMasqName,
						ConditionGetterFunc(DNSMasqConditionGetter),
						condition.DeploymentReadyCondition,
					...
					Expect(instance.Status.ReadyCount).To(BeNumerically(">", 0))
				})
			})
		*/
