...
var _ = Describe("Service controller", func() {
	...
	When("A Service is created with dnsmasq annotation", func() {
		...
		It("should have created a DNSData with hosts data", func() {
			...
			// seems a wait is required, otherwise the spec is missing data?!?
			time.Sleep(1000 * time.Millisecond)

			dnsdata := GetDNSData(svcDNSData)
			Expect(dnsdata).To(Not(BeNil()))
			Expect(dnsdata.Spec.DNSDataLabelSelectorValue).To(Equal("dnsdata"))
			Expect(dnsdata.Spec.Hosts).To(Not(BeNil()))
			Expect(dnsdata.Spec.Hosts[0].IP).To(Equal("172.20.0.80"))
			Expect(dnsdata.Spec.Hosts[0].Hostnames[0]).To(Equal(fmt.Sprintf("some-service.%s.svc", namespace)))
		})
	})
	...
	When("A Service is created without dnsmasq annotation", func() {
		...
		It("should not have created a DNSData without hosts data", func() {
			...
			Expect(dnsdata).To(Not(BeNil()))
			Expect(dnsdata.Spec.DNSDataLabelSelectorValue).To(Equal("dnsdata"))
			Expect(dnsdata.Spec.Hosts).To(BeNil())
		})
	})
})
