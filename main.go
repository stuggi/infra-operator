...
func main() {
	...
	if err = (&networkcontrollers.NetConfigReconciler{
		Client:  mgr.GetClient(),
		Kclient: kclient,
		Log:     ctrl.Log.WithName("controllers").WithName("NetConfig"),
		Scheme:  mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "NetConfig")
		os.Exit(1)
	}
	...
	if err = (&networkcontrollers.ReservationReconciler{
		Client:  mgr.GetClient(),
		Kclient: kclient,
