...
//+kubebuilder:rbac:groups=network.openstack.org,resources=netconfigs,verbs=get;list
...
func (r *IPSetReconciler) reconcileDelete(ctx context.Context, instance *networkv1.IPSet, helper *helper.Helper) (ctrl.Result, error) {
	...
	if err := r.delFinalizerOnReservation(ctx, res, helper); err != nil {
		return ctrl.Result{}, err
	}
...
func (r *IPSetReconciler) reconcileNormal(ctx context.Context, instance *networkv1.IPSet, helper *helper.Helper) (ctrl.Result, error) {
	...
	if len(netcfgs.Items) > 0 {
		...
		if err != nil {
			// TODO add condition
			return ctrl.Result{}, fmt.Errorf("failed to create new reservation for %s - %w", instance.Name, err)
		}
...
func (r *IPSetReconciler) delFinalizerOnReservation(ctx context.Context, res *networkv1.Reservation, helper *helper.Helper) error {
	if controllerutil.RemoveFinalizer(res, helper.GetFinalizer()) {
...
func (r *IPSetReconciler) ensureReservation(
	...
	for _, ipsetNet := range ipset.Spec.Networks {
		...
		ipDetails := ipam.AssignIPDetails{
			...
			SubNetName:  string(ipsetNet.SubnetName),
