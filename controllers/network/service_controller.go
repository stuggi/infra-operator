...
func (r *ServiceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// Get a list of all DNSMasqs to trigger reconcile only for these namespaces
	...
	for _, dnsmasq := range dnsmasqs.Items {
		dnsHosts, err := r.getServiceDNSData(ctx, dnsmasq.Namespace)
		if err != nil {
		...
		err = r.createOrPatchDNSData(ctx, dnsmasq.Namespace, &dnsmasq.ObjectMeta, dnsmasq.Spec.DNSDataLabelSelectorValue, sortedDNSHosts)
		if err != nil {
			return ctrl.Result{}, err
		}
	}
...
func (r *ServiceReconciler) createOrPatchDNSData(
	...
	err := r.Client.Get(ctx, types.NamespacedName{Name: owner.GetName(), Namespace: owner.GetNamespace()}, instance)
	if err != nil {
		return err
	}
	...
	op, err := controllerutil.CreateOrPatch(ctx, r.Client, svcDNSData, func() error {
		...
		svcDNSData.Spec.DNSDataLabelSelectorValue = dnsDataLabelSelectorValue
		svcDNSData.Spec.Hosts = svcDNSHosts

		err := controllerutil.SetControllerReference(instance, svcDNSData, r.Scheme)
		if err != nil {
			return err
		}
