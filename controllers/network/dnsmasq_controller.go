...
func (r *DNSMasqReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, _err error) {
	...
	defer func() {
		...
		if allSubConditionIsTrue(instance.Status.Conditions) {
...
func (r *DNSMasqReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	dnsmasqFN := handler.EnqueueRequestsFromMapFunc(func(o client.Object) []reconcile.Request {
		...
		// Get a list of all DNSMasq to trigger reconcile for the one in the same namespace
	...
	p := predicate.Funcs{
		UpdateFunc: func(e event.UpdateEvent) bool {
			// The object doesn't contain label dnsmasq.DNSDataLabelSelector, so the event will be
			// ignored.
			if _, ok := e.ObjectOld.GetLabels()[dnsmasq.DNSDataLabelSelector]; !ok {
				return false
			}
		...
		CreateFunc: func(e event.CreateEvent) bool {
			if _, ok := e.Object.GetLabels()[dnsmasq.DNSDataLabelSelector]; !ok {
				return false
			}
...
func (r *DNSMasqReconciler) reconcileNormal(ctx context.Context, instance *networkv1.DNSMasq, helper *helper.Helper) (ctrl.Result, error) {
	...
	labelSelectorMap := map[string]string{dnsmasq.DNSDataLabelSelector: strings.ToLower(instance.Spec.DNSDataLabelSelectorValue)}
	...
	inputHash, err := hashOfInputHashes(configMapVars)
	if err != nil {
		instance.Status.Conditions.Set(condition.FalseCondition(
			condition.ServiceConfigReadyCondition,
			condition.ErrorReason,
